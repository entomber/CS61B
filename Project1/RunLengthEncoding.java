...
public class RunLengthEncoding implements Iterable {
	...
	public RunLengthEncoding(int width, int height, int[] red, int[] green,
		...
		if (red.length == 0 || green.length == 0 || 
				blue.length == 0 || runLengths.length == 0) {
			System.err.println("Run/pixel intensity array length(s) equal to zero.");
			return;
		} 

		if (red.length != green.length || red.length != blue.length ||
				red.length != runLengths.length) {
			System.err.println("Run/pixel intensity arrays not same length.");
			return; 
		}
		...
		if (sum != width * height) {
			System.err.println("Sum of run lengths does not equal width * height.");
			return;
		}
		...
		for (int i = 0; i < runLengths.length; i++) {
			if (red[i] < 0 || red[i] > 255 || green[i] < 0 || green[i] > 255 ||
					blue[i] < 0 || blue[i] > 255) {
				System.err.println("pixel intensity is out of range [0, 255].");
				return;
			}
	...
	public PixImage toPixImage() {
		...
		while (it.hasNext()) {
			...
			while (runLength > 0) {
				...
				x++;
				if (x == width) { // x greater than index value, increment y and reset x
					y++;
					x = 0;
				}
				runLength--;
	...
	public void setPixel(int x, int y, short red, short green, short blue) {
		...
		// find the run pixel is in by comparing its pixel position to the
		...
		if (sumRunLength + 1 == pixelPosition) {
			...
			if (run.prev != runs.head) {
				// single pixel run
				if (run.item[0] == 1) {
					// pixel matches previous and next run's pixels
				...
				// run length > 1
					...
					// matches previous run's pixels
					if (run.prev.item[1] == red && run.prev.item[2] == green &&
						 	run.prev.item[3] == blue) {
						run.prev.item[0]++;
						run.item[0]--;
					} 
					// doesn't match previous run's pixels
					else {
						DListNode before = new DListNode(newRun, run.prev, run);
						run.item[0]--;
						// link new node
		...
		// pixel is last pixel of run (run length > 1)
			...
			// insert new run after
				...
				DListNode after = new DListNode(newRun, run, run.next);
				run.item[0]--;
				// link new node
				after.prev = run;
				after.next = run.next;
		...
		// pixel somewhere in middle
			...
			DListNode mid = new DListNode(newRun, start, null);
			DListNode end = new DListNode(endRun, mid, next);
	...
	/*  goes through all pixels of input array, changing one
			pixel at a time */
